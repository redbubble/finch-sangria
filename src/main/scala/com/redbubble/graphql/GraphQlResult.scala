...
/**
  ...
  */
final case class SuccessfulGraphQlResult(underlying: Json, context: Any = Unit) extends GraphQlResult

/**
  * The result of a failed GraphQL query execution, where the failure was prior to the execution (i.e. bad query).
  */
final case class ClientErrorGraphQlResult(underlying: Json, errors: Seq[RegisteredError] = Seq.empty, context: Any = Unit)
...
/**
  ...
  */
final case class BackendErrorGraphQlResult(underlying: Json, errors: Seq[RegisteredError] = Seq.empty, context: Any = Unit)
    extends GraphQlResult
